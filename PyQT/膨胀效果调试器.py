...
class ImageDisplayWidget(QWidget):
    ...
    def initUI(self):
        ...
        self.slider_value_label2 = QLabel("膨胀次数: 0")
        controls_layout = QHBoxLayout()
        self.add_control(controls_layout, "矩形核大小（kernel）:", self.slider, self.slider_value_label)
        controls_layout2 = QHBoxLayout()
        self.add_control(controls_layout2, "膨胀次数（iterations）:", self.slider2, self.slider_value_label2)
    ...
    def process_image(self):
        ...
        self.slider_value_label2.setText(f"膨胀次数: {iterations}")
        kernel = np.ones((kernel_size, kernel_size), dtype=np.uint8)
        processed_image = cv2.dilate(self.image, kernel, iterations=iterations)

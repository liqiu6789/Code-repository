...
class VGGish(VGG):
    def __init__(self, urls, device=None, pretrained=True, preprocess=True,
                 postprocess=True, progress=True, model_dir=None):
        super().__init__(make_layers())
        if pretrained:
            state_dict = hub.load_state_dict_from_url(urls['vggish'], progress=progress, model_dir=model_dir)
        ...
        if self.postprocess:
            ...
            if pretrained:
                state_dict = hub.load_state_dict_from_url(urls['pca'], progress=progress, model_dir=model_dir)
